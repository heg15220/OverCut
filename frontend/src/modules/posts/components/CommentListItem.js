...
const CommentListItem = ({ key, comment, level }) => {
    ...
    return (
        <div>
            <div className="card my-2 overflow-visible text-start" style={{ maxWidth: 350, marginLeft: `${15 * level}px`, marginRight: '10px', position: 'relative' }}>
                <div className="d-flex">
                    ...
                    <text class="my-3 mx-2" style={{ color: '#00000F' }}>{comment.userName}</text>
                </div>

                <text className="card-body" style={{ marginBottom: 14 }}>{comment.content}</text>
                {user &&
