...
import "./Top10Game.css"; // reutiliza los estilos de gridGame
...
const SearchPilotInput = () => {
  ...
  const gameId = useSelector(getTop10GameId);
  const validated = useSelector(getTop10Validated);
  const suggestions = useSelector(getTop10Suggestions);
  ...
  useEffect(() => {
    const delay = setTimeout(() => {
      if (input.length >= 2) {
        ...
        dispatch({ type: "SET_TOP10_SUGGESTIONS", suggestions: [] });
  ...
  const handleSelect = (name) => {
    dispatch(actions.validatePilot(gameId, name, (result) => {
      ...
      if (availablePosition != null) {
        dispatch({
          type: "VALIDATE_TOP10_SLOT_COMPLETED",
