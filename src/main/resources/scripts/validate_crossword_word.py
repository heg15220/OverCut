...
def validate_driver(connection, word, clue):
    try:
        ...
        result = connection.execute(query, {"name": f"%{word}%"}).fetchone()
        if not result:
            return False
        if "campeón" in clue.lower() or "champion" in clue.lower():
            ...
            """)
            standings = connection.execute(query2, {"driverId": result["driverId"]}).fetchone()
            return standings and standings["titles"] > 0
        return True
