...
public interface UserNotificationDao extends CrudRepository<UserNotification, Long> {
    ...
    @Query("SELECT unn FROM UserNotification unn WHERE unn.user.id = :userId AND unn.read = false")
    Slice<UserNotification> findUnreadByUserId(@Param("userId") Long userId, Pageable pageable);
